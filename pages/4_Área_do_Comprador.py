...
def load_catalog():
    ...
    if not os.path.exists(CATALOG_FILE_PATH):
        st.error(
            f"Erro: Arquivo de catálogo não encontrado em `{CATALOG_FILE_PATH}`. Execute os scripts de geração primeiro."
        )
        return None
...
if catalog:
    ...
    selected_file = st.selectbox(
        "**1. Selecione o conjunto de dados que deseja editar:**", file_names
    )

    if selected_file:
        # Encontrar a entrada do catálogo para o arquivo selecionado
        selected_entry = next(
            (item for item in catalog if item["file_name"] == selected_file), None
        )
        ...
        st.caption(
            "Explique o que este conjunto de dados representa. Ex: Cadastro de produtos, vendas diárias, etc."
        )
        ...
        st.caption(
            "Para cada coluna, explique o que ela significa. O agente usará isso para encontrar a informação certa."
        )
