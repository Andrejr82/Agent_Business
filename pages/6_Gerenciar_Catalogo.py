...
def show_catalog_manager():
    ...
    if catalog:
        selected_file_name = st.selectbox(
            "Selecione a Fonte de Dados para Editar", [d["file_name"] for d in catalog]
        )
        selected_index = next(
            (i for i, d in enumerate(catalog) if d["file_name"] == selected_file_name),
            None,
        )
        ...
        if selected_index is not None:
            ...
            with st.form("edit_data_source_form"):
                edited_description = st.text_area(
                    "Descrição", value=selected_entry["description"]
                )
                ...
                for col_name, col_desc in selected_entry["column_descriptions"].items():
                    edited_columns[col_name] = st.text_input(
                        f"Descrição da Coluna: {col_name}",
                        value=col_desc,
                        key=f"edit_col_{selected_file_name}_{col_name}",
                    )
                ...
                if edit_submitted:
                    ...
                    if new_col_name and new_col_desc:
                        selected_entry["schema"][
                            new_col_name
                        ] = "object"  # Default type
                        selected_entry["column_descriptions"][
                            new_col_name
                        ] = new_col_desc
                        selected_entry["column_count"] = len(
                            selected_entry["schema"]
                        )  # Update count

                    save_catalog(catalog)
                    st.success(
                        f"Fonte de dados '{selected_file_name}' atualizada com sucesso!"
                    )
                    st.rerun()
    else:
