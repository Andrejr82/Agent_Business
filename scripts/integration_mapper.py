...
FILE_OBSOLETE_CANDIDATES = ["core/llm_adapter.py", "core/transformer_adapter.py"]
...
def map_integrations(root_dir):
    ...
    method_usage = {
        f"{file}::{method}": []
        for file, methods in OBSOLETE_CANDIDATES.items()
    ...
    for subdir, _, files in os.walk(root_dir):
        if any(
            excluded in subdir for excluded in ["venv", ".pytest_cache", "__pycache__"]
        ):
            continue
        for file in files:
            if file.endswith(".py"):
...
def generate_report(method_usage, file_usage):
    ...
    report += "4.  **Agente de Ferramentas (`core/agents/tool_agent.py`)**: Atua como um wrapper para o modelo de linguagem (LLM), tanto para extrair filtros quanto para gerar respostas em linguagem natural.\n"
    ...
    for key, callers in method_usage.items():
        if not callers:
            ...
            report += "  - **Status:** **Obsoleto.** A funcionalidade é coberta pelo método `search_products` que usa o LLM.\n\n"
    ...
    for file, importers in file_usage.items():
        if not importers:
            ...
            report += f"- **Arquivo:** `{file}`\n"
            report += "  - **Status:** **Obsoleto.** A API do LLM é chamada diretamente, tornando este adaptador desnecessário.\n\n"
