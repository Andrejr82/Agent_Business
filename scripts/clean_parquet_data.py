...
def normalize_column_name(col_name):
    ...
    no_special_chars = (
        only_ascii.replace(" ", "_").replace(".", "").replace("%", "PERCENT")
    )
...
def clean_parquet_file(file_path, output_dir):
    ...
    try:
        ...
        if "ADMAT_SEMVENDAS" in base_name and "DATACRIALINHAVERDE" in df.columns:
            ...
            df["DATACRIALINHAVERDE"] = pd.to_datetime(
                df["DATACRIALINHAVERDE"], errors="coerce"
            )
        ...
        print(
            f"    - Encontradas {len(object_cols)} colunas do tipo 'object' para otimizar."
        )
        for col in object_cols:
            try:
                ...
                print(
                    f"      - AVISO: Nao foi possivel otimizar a coluna '{col}'. Erro: {inner_e}"
                )
                continue
