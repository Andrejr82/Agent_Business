...
def rebuild_and_clean_data(source_path, admat_path, destination_path):
    ...
    try:
        ...
        print(
            f"--- Carregando dados do comprador de: {os.path.basename(admat_path)} ---"
        )
        ...
        print(
            f"Coluna COMPRADOR adicionada. Total de valores não nulos: {df['COMPRADOR'].notna().sum()}"
        )
...
if __name__ == "__main__":
    ...
    destination_file = os.path.join(
        os.getcwd(), "data", "parquet_cleaned", "ADMAT_REBUILT.parquet"
    )
