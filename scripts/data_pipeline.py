...
class DataPipeline:
    ...
    def __init__(self, db_params: Dict[str, str]):
        ...
        self.output_dir = os.path.join(
            os.path.dirname(__file__), "..", "data", "parquet_cleaned"
        )
    ...
    def _extract_table_to_parquet(self, table_name: str, output_filename: str):
        ...
        logging.info(
            f"Iniciando extração da tabela '{table_name}' para '{output_path}'..."
        )
        ...
        try:
            with self.engine.connect() as connection:
                ...
                if df.empty:
                    logging.warning(
                        f"A tabela '{table_name}' está vazia ou não retornou dados."
                    )
                    return

                df.to_parquet(output_path, index=False, compression="snappy")
                logging.info(
                    f"Tabela '{table_name}' extraída com sucesso. {len(df)} linhas salvas em '{output_path}'."
                )

        except Exception as e:
            logging.error(
                f"Falha ao extrair a tabela '{table_name}'. Erro: {e}", exc_info=True
            )
            raise
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Pipeline de Dados para o Caçulinha BI."
    )
    ...
    parser.add_argument(
        "--password", required=True, help="Senha para o banco de dados."
    )
