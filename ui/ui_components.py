...
def get_image_download_link(fig, filename, text):
    ...
    try:
        ...
        href = f'<a class="download-button" href="data:image/png;base64,{b64}" download="{filename}.png" role="button" aria-label="{text}">{text}</a>'
        return href
        ...
        href = f'<a class="download-button" href="data:text/html;base64,{b64}" download="{filename}.html" role="button" aria-label="{text} (HTML)">{text} (HTML)</a>'
        st.warning(
...
def get_csv_download_link(df, filename, text):
    ...
    href = f'<a class="download-button" href="data:file/csv;base64,{b64}" download="{filename}.csv" role="button" aria-label="{text}">{text}</a>'
    return href
