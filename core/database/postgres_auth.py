...
def initialize_db():
    ...
    Para PostgreSQL/Supabase, verifica a conexão.
    """
    db_manager = get_db_manager()
    success, message = db_manager.test_connection()
    if not success:
        logger.error(f"Falha ao inicializar DB para autenticação: {message}")
        raise ConnectionError(f"Não foi possível conectar ao PostgreSQL: {message}")
    logger.info("PostgreSQL DB para autenticação inicializado e conectado.")
...
def verify_user(username: str, password: str) -> Optional[Dict[str, str]]:
    ...
    db_manager = get_db_manager()
    try:
        # PostgreSQL usa :param para bind parameters com SQLAlchemy text()
        query = "SELECT id, username, password_hash, role, ativo FROM users WHERE username = :username AND ativo = true"
        ...
        logger.error(f"Erro ao verificar usuário no PostgreSQL: {e}", exc_info=True)
        return None

def criar_usuario(username: str, password: str, role: str) -> None:
    """
    Cria um novo usuário no banco de dados PostgreSQL.
    """
    db_manager = get_db_manager()
    ...
    try:
        with db_manager.get_session_context() as session:
            ...
            insert_query = "INSERT INTO users (username, password_hash, role, ativo) VALUES (:username, :password_hash, :role, true)"
...
def set_user_status(user_id: int, is_active: bool) -> None:
    ...
    try:
        with db_manager.get_session_context() as session:
            # PostgreSQL usa booleano nativo
            status_value = True if is_active else False
