    ...
    try:
        ...
        for entry in catalog:
            ...
            if entry["description"] == "(Descrição a ser preenchida)":
                file_desc = generate_description_from_name(
                    entry["file_name"].replace(".parquet", "")
                )
            ...
            for col_name, col_desc in entry["column_descriptions"].items():
                if col_desc == "(Descrição a ser preenchida)":
                    entry["column_descriptions"][col_name] = (
                        generate_description_from_name(col_name)
                    )
        ...
        print(
            f"[!] Erro: O arquivo de catálogo de entrada não foi encontrado em {input_path}"
        )
