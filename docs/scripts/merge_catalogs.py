...
def merge_catalogs():
    ...
    try:
        ...
        admat_rebuilt_new = next(
            (
                item
                for item in data_catalog
                if item["file_name"] == "ADMAT_REBUILT.parquet"
            ),
            None,
        )
        admat_rebuilt_old = next(
            (
                item
                for item in focused_catalog
                if item["file_name"] == "ADMAT_REBUILT.parquet"
            ),
            None,
        )
        ...
        if admat_rebuilt_new and admat_rebuilt_old:
            ...
            description = admat_rebuilt_old.get(
            ...
            new_column_descriptions = {
                to_snake_case(old_col): desc
                for old_col, desc in old_column_descriptions.items()
            }
            ...
            print(
                "Não foi possível encontrar a entrada para 'ADMAT_REBUILT.parquet' em um dos catálogos."
            )
