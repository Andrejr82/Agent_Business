...
def analyze_parquet_file(file_path):
    ...
    try:
        ...
        null_df = pd.DataFrame(
            {"Valores Nulos": null_counts, "Porcentagem (%)": null_percentages}
        )
        print(
            null_df[null_df["Valores Nulos"] > 0].sort_values(
                by="Porcentagem (%)", ascending=False
            )
        )
        ...
        for col in df.select_dtypes(include=["object"]).columns:
            try:
                ...
                print(
                    f"  - AVISO: A coluna '{col}' é do tipo 'object', mas parece conter apenas dados numéricos."
                )
