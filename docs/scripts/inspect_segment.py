...
def inspect_segment_categories(
    file_path, segment_column, segment_value, category_column
):
    """
    ...
    try:
        ...
        if segment_column in df.columns and category_column in df.columns:
            ...
            print(
                f"Uma das colunas '{segment_column}' ou '{category_column}' não foi encontrada."
            )
...
if __name__ == "__main__":
    parquet_file = os.path.join(
        os.getcwd(), "data", "parquet_cleaned", "Filial_Madureira.parquet"
    )
