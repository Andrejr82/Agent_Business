...
def restructure_parquet(source_filename, dest_filename, column_mapping):
    ...
    try:
        ...
        print(
            f"Lido {source_filename} com {len(source_df.columns)} colunas e {len(source_df)} linhas."
        )
        ...
        if "FORALINHA" in restructured_df.columns:
            ...
            if (
                "FORALINHA" in source_df.columns
            ):  # Verifica se a coluna existia antes do mapeamento
                pass  # Nenhuma transformação por enquanto, apenas copiando

        print(
            f"Colunas reestruturadas. O novo DataFrame tem {len(restructured_df.columns)} colunas."
        )
...
if __name__ == "__main__":
    restructure_parquet(
        "Filial_Madureira.parquet", "ADMAT_structured.parquet", map_admat
    )
    restructure_parquet(
        "ADMAT_SEMVENDAS.parquet",
        "ADMAT_SEMVENDAS_structured.parquet",
        map_admat_semvendas,
    )
