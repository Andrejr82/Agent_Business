...
def generate_cleanup_script(
    files_to_delete, script_name="delete_unnecessary_files.bat"
):
    project_root = get_project_root()
...
"""
    ...
    for item_path in files_to_delete:
        if item_path.endswith("/") or os.path.isdir(
            item_path
    ...
    print(
        "To run, open Command Prompt or PowerShell, navigate to the project root, and execute:"
    )
    print(f"  {script_name}")
