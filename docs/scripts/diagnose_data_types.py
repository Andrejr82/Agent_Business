...
def diagnose_numeric_columns(file_path):
    ...
    try:
        ...
        potential_numeric_cols = [
            ...
            if "VENDA" in col.upper()
            or "QTD" in col.upper()
            or "PRECO" in col.upper()
            or "VALOR" in col.upper()
            or "MES" in col.upper()
            or "202" in col
        ]
        ...
        if not non_numeric_values:
            print(
                "Nenhum valor de texto encontrado nas colunas numéricas. O arquivo parece limpo."
            )
        else:
            print("\nRelatório de valores não numéricos encontrados:")
            for value, count in non_numeric_values.items():
                print(
                    f"  - Valor: '{value}' (encontrado em {count} coluna(s) distinta(s)) "
                )
